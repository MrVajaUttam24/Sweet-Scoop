...
<head>
    ...
    <title>Sweet Scoop - Add New Topping</title>
    ...
    <style>
        ...
        * {
        ...
        .container {
            max-width: 600px;
            margin: 30px auto; /* Adjusted margin for space from nav */
        ...
        footer h2 {
            ...
            text-align: left; /* Align footer heading left */
        }
        ...
        @media (max-width: 768px) {
            ...
            footer div {
                flex: 1 1 100%; /* Stack footer columns on small screens */
                text-align: center;
            }
...
<body>
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if (move_uploaded_file($imageTmp, $imagePath)) {
            $stmt = $conn->prepare("INSERT INTO topings (name, image, price) VALUES (?, ?, ?)");
            $stmt->bind_param("ssi", $name, $imagePath, $price);

            if ($stmt->execute()) {
                $message = "<div class='alert success'><strong>Success:</strong> Topping added successfully.</div>";
            } else {
                $message = "<div class='alert error'><strong>Error:</strong> " . htmlspecialchars($stmt->error) . "</div>";
            }

            $stmt->close();
        } else {
            $message = "<div class='alert error'><strong>Error:</strong> Failed to upload image.</div>";
        }
    ...
    <div class="container">
        ...
        <?php if ($imagePath && file_exists($imagePath)) : ?>
            <div class="preview">
                ...
                <img src="<?php echo $imagePath; ?>" alt="Topping Image">
            </div>
        <?php endif; ?>
    </div>
