...
<head>
    ...
    <title>Manage Toppings - Sweet Scoop</title>
    ...
    <style>
        ...
        * {
        ...
        body {
            font-family: 'Poppins', sans-serif; /* Changed to Poppins as it's the primary content font */
            background: linear-gradient(to right top, #e7f5ff, #eef8ff);
            min-height: 100vh;
            overflow-x: hidden;
            display: flex; /* Added for main content layout */
            flex-direction: column; /* Added for main content layout */
            align-items: center; /* Added for main content layout */
            padding-top: 2rem; /* Initial padding for space from top */
        }
        ...
        nav {
            ...
            border-radius: 30px;
            margin: 0 auto 3rem auto; /* Adjusted margin to add space below nav */
            padding: 1rem 2rem;
            max-width: 100%;
            width: calc(100% - 4rem); /* To keep it within view on smaller screens */
            display: flex;
        ...
        h1 {
            ...
            color: #333;
            margin-bottom: 40px;
            margin-top: 2rem; /* Space between nav and H1 */
        }
        ...
        footer a:hover {
            color: var(--violet); /* Example hover color for footer links */
        }
        ...
        @media (max-width: 768px) {
            ...
            .admin-button {
                ...
                margin-top: 1rem; /* Space for the button */
            }
...
<body>
    ...
    if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['delete_id'])) {
        ...
        $deleteSql = "DELETE FROM topings WHERE sr = ?";
    ...
    // Fetch toppings data
    $sql = "SELECT sr, name, image, price FROM topings";
    ...
    if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_assoc()) {
            echo '<tr>
                    ...
                    <td data-label="Image"><img src="' . htmlspecialchars($row["image"]) . '" alt="Topping Image" /></td>
                    <td data-label="Price">₹' . number_format($row["price"], 2) . '</td>
                    <td data-label="Actions" class="action-buttons">
                        <a href="edit_topping.php?sr=' . $row["sr"] . '" class="edit-btn">Edit</a>
        ...
        echo "<p style='font-size: 1.2em;'>No toppings found.</p>";
    }
